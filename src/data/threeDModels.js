const threeDModels = [
  {
    id: 0,
    name: 'winterfell',
    description: 'winterfell',
    position: [14.542365659089342, 26.541643634608064],
    url: '/static/3dmodels/winterfell/scene.gltf',
    size: 20
  },
  {
    id: 1,
    name: 'castleblack',
    description: 'castleblack',
    position: [18.82630959886862, 34.830997261926996],
    url: '/static/3dmodels/castleblack/scene.gltf',
    size: 10
  },
  {
    id: 2,
    name: 'weirwood',
    description: 'weirwood beyond the wall',
    position: [13.836566235047457, 27.496264003600388],
    url: '/static/3dmodels/weirwood/scene.gltf',
    size: 1000
  },
  {
    id: 3,
    name: 'castleblack',
    description: 'weirwood in winterfell',
    position: [15.5100820713146, 39.09968493908772],
    url: '/static/3dmodels/weirwood/scene.gltf',
    size: 1000
  }
]

export default threeDModels
